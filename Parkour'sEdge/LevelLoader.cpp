...
void LoadActor(const rapidjson::Value& actorValue, Game* game, Actor* parent)
{
	if (actorValue.IsObject())
	{
		...
		if (type == "Block")
		{
			...
			{
				block->SetMirrorStatus(isMirror);
			}
			bool isRotate;
			if (GetBoolFromJSON(actorValue, "rotates", isRotate))
			{
				block->SetRotatingStatus(isRotate);
			}
		}
		else if (type == "Player")
		{
		...
		else if (type == "Checkpoint")
		{
			...
			std::string lvl;
			if (GetStringFromJSON(actorValue, "level", lvl))
			{
				checkpoint->SetLevelString(lvl);
			}
			std::string checkpointText;
			if (GetStringFromJSON(actorValue, "text", checkpointText))
			{
				checkpoint->SetText(checkpointText);
			}
		}
		else if (type == "Coin")
		{
		...
		{
			SecurityCamera* cam = new SecurityCamera(game, parent);
			actor = cam;
			float interp, pause;
			Quaternion sq, eq;
			...
			if (GetFloatFromJSON(actorValue, "interpTime", interp))
			{
				...
				{
					cam->SetInterpPauseTime(interp, pause);
				}
			}
		}
