...
Mesh::Mesh(AssetManager* pAssetManager, const VertexBuffer* pVertexBuffer, const Shader* pShader)
	:mAssetManager(pAssetManager), mShader(pShader), mVertexBuffer(pVertexBuffer)
{
...
bool Mesh::Load(const WCHAR* fileName)
{
	...
	//skinning stuff
	if (shaderName == L"Skinned") skinned = true;
	...
	bool isSkinned = (L"Skinned" == shaderName);
